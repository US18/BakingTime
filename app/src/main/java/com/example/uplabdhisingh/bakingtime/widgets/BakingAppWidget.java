...
import com.example.uplabdhisingh.bakingtime.BakingActivityDetails;
import com.example.uplabdhisingh.bakingtime.R;

import static com.example.uplabdhisingh.bakingtime.widgets.UpdateBakingService.FROM_ACTIVITY_INGREDIENTS_LIST;
...
public class BakingAppWidget extends AppWidgetProvider {
    ...
    static void updateAppWidget(Context context, AppWidgetManager appWidgetManager,
        ...
        Intent appIntent = new Intent(context, BakingActivityDetails.class);
        appIntent.addCategory(Intent.ACTION_MAIN);
        appIntent.addCategory(Intent.CATEGORY_LAUNCHER);
        appIntent.addFlags(Intent.FLAG_ACTIVITY_BROUGHT_TO_FRONT | Intent.FLAG_ACTIVITY_SINGLE_TOP);
        PendingIntent appPendingIntent = PendingIntent.getActivity(context, 0, appIntent, PendingIntent.FLAG_UPDATE_CURRENT);
        views.setPendingIntentTemplate(R.id.baking_app_widget, appPendingIntent);

        // Set the GridWidgetService intent to act as the adapter for the GridView
        Intent intent = new Intent(context, GridWidgetService.class);
        views.setRemoteAdapter(R.id.baking_app_widget, intent);
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action.equals("android.appwidget.action.APPWIDGET_UPDATE2")) {
            ...
            appWidgetManager.notifyAppWidgetViewDataChanged(appWidgetIds, R.id.baking_app_widget);
